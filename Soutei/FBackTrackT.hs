...
instance Monad m => Applicative (Stream m) where
  ...
  (<*>) appF target = Stream $ (runAppF <$> unStream target) <*> (unStream appF)
   where
     runF _ Nil = Nil
     runF f (One a) = One $ f a
     runF f (Choice a sa) = Choice (f a) (fmap f sa)
     runF f (Incomplete i) = Incomplete $ fmap f i
     runAppF _ Nil = Nil
     runAppF sa (One f) = runF f sa
     runAppF sa (Incomplete i) = Incomplete . Stream $ runAppF sa <$> (unStream i)
     runAppF sa (Choice f sf ) = mplus' (runF f sa) (Stream $ runAppF sa <$> (unStream sf) )
     mplus' Nil          m2 = Incomplete m2
     mplus' (One a)      m2 = Choice a m2
     mplus' (Choice a r) m2 = Choice a (mplus m2 r) -- interleaving!


--         (Choice f sf) -> return $  (pure $ runF f target) -- `mplus` (yield (f `fmap` sf ))
instance Monad m => Alternative (Stream m) where
  empty = Stream $ return Nil
  ...
  mplus m1 m2 = Stream (unStream m1 >>= mplus')
   ...
   mplus' r@(Incomplete i) = unStream m2 >>= \r' ->
      case r' of
              ...
              Choice b rslt -> return $ Choice b (mplus i rslt)
...
runM' :: forall (m :: * -> *) a.
               Monad m =>
               Maybe Int -> Maybe Int -> StreamE m a -> m [a]
...
runM' (Just 0) _ (Incomplete _r) = return []     -- exhausted depth
...
test_flaz = runIdentity $ runM Nothing (Just 15) (flaz (5::Integer))
